...
export default function Programmers() {
  ...
  const [idOfFeaturedP, setIdOfFeaturedP] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    let curProgrammer = programmers.filter(dev => dev.id === idOfFeaturedP)
    return curProgrammer[0].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idOfFeaturedP ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setIdOfFeaturedP(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
